...
#define BackendExeName "vigil-backend.exe"
...
OutputDir=output
OutputBaseFilename=Vigil-Setup
...
Root: HKA; Subkey: "Software\Microsoft\Windows\CurrentVersion\Run"; ValueType: string; ValueName: "Vigil"; ValueData: """{app}\tray\{#AppExeName}"""; Flags: uninsdeletevalue
...
var
  ResultCode: Integer;
begin
  Exec('cmd.exe', '/C netstat -ano | findstr ":7422 " | findstr "LISTENING" > "%TEMP%\vigil-port-check.txt"',
  ...
  if not Result then
    MsgBox('Port 7422 is already in use on this machine.' + #13#10 +
           ...
           'If Vigil is already installed and running, uninstall it first.',
           mbError, MB_OK);
end;
